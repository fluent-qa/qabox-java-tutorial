...
public class HttpServletRequestFilter extends GenericFilterBean {
    ...
    private final BoxSecurityProp boxSecurityProp;

    private static final String CONTENT_TYPE_JSON = "application/json";

    public HttpServletRequestFilter(BoxSecurityProp boxSecurityProp) {
        this.boxSecurityProp = boxSecurityProp;
    }
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        if (boxSecurityProp.isRecordOperateLog()) {
            ...
            if (servletRequest instanceof HttpServletRequest) {
                ...
                if (request.getServletPath().contains(BoxRestPath.ERUPT_API)) {
                    if (null != request.getContentType() && CONTENT_TYPE_JSON.equals(request.getContentType())) {
                        EruptRequestWrapper eruptRequestWrapper =
